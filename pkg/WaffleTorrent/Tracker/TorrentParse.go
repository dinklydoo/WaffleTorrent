...
/*
	...
	Example Bencode for Debian (the superior linux distro!!)
	(formatted nicely *note this would all be in a single line*):
...
func ParseTorrent(data *[]byte) (*WaffleTorrent.Torrent, error) {
	...
	files := make([]*WaffleTorrent.File, 0)
	total_length := int64(0)

	// single file context
	if info.Length > 0 {
		files = append(files, &WaffleTorrent.File{
...
func ParseTorrentFromFile(path string) (*WaffleTorrent.Torrent, error) {
